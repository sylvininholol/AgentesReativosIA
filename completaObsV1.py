...
class Ambient:
    ...
    def agentAct(self):
        action = self.vacuum.act(self.rooms)
        if action == "Limpando...":
            print("Limpando...")
            self.rooms[self.vacuum.position].clean()
...
class Vacuum:
    ...
    def act(self, rooms):
        dirty_rooms = [i for i, room in enumerate(rooms) if room.dirtyBool == 1]
        if dirty_rooms:
            closest_dirty_room = min(dirty_rooms, key=lambda x: abs(x - self.position))
            if closest_dirty_room > self.position:
                self.position += 1
                print("Indo para a direita")
            elif closest_dirty_room < self.position:
                self.position -= 1
                print("Indo para a esquerda")
            else:
                return "Limpando..."
        else:
            print("Não há sujeira para limpar")
